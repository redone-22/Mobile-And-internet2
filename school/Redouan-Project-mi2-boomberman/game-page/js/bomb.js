...
$(document).keydown(function (b) {
    if ((b.which) === space) {
        if (bomInGame <= numberBom) {playerPosition = $('#bomber').position(); playerPositions = {top: playerPosition.top, left: playerPosition.left}; $("#gamePlatform").append("<img src='img/Smoothie_Smash_Bomb.gif' alt='bomb' id='bomb" + bomhist + "'>");
                            $("#bomb" + bomhist).css({position: 'absolute', top: playerPositions.top + 5, left: playerPositions.left + 15, height: '50px', width: '50px'}); bomInGame = bomInGame + 1; bomhist = bomhist + 1;   setTimeout(function b1() { $("#bomb" + bomexplosion).attr('src', 'img/bom.png'); $("#bomb" + bomhist).css({height: '100px', width: '100px'}); bomexplosion = bomexplosion + 1; bomInGame = bomInGame - 1; setTimeout(function () {$("#bomb" + bomhist).remove(); }, 5500); }, 5000);
